import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import math

# Pengaturan dasar halaman
st.set_page_config(page_title="Dashboard Finansial", layout="wide")
st.title(":blue[Dashboard Simulasi Finansial]")

# --- SINKRONISASI VARIABEL ANTAR TAB (SESSION STATE) ---
if 'modal_awal' not in st.session_state: st.session_state.modal_awal = 300000000
if 'tambahan_tahunan' not in st.session_state: st.session_state.tambahan_tahunan = 50000000
if 'dividen_tahun' not in st.session_state: st.session_state.dividen_tahun = 8.0

tab1, tab2, tab3 = st.tabs(["Kalkulator Pinjaman Institusi", "Simulasi Investasi Bertahap", "Analisis Ekuitas Bersih"])

# ==========================================
# TAB 1: KALKULATOR PINJAMAN INSTITUSI
# ==========================================
with tab1:
    st.header(":orange[Kalkulator Pinjaman Berbasis Data Pasar]")
    col1, col2 = st.columns([1, 3])
    with col1:
        st.subheader("Parameter Institusi")
        kategori_bank = st.selectbox("Pilih Institusi Perbankan", list(db_bank.keys()) if 'db_bank' in locals() else ["Bank Himbara (BUMN)", "Bank Swasta Utama"])
        
        # Default database jika belum terdefinisi global
        db_rates = {
            "Bank Himbara (BUMN)": {"KUR": 6.00, "KPR_Promo": 6.00, "KPR_Float": 12.50},
            "Bank Swasta Utama": {"KTA": 14.50, "KPR_Promo": 5.50, "KPR_Float": 12.00}
        }
        
        jenis_kredit = st.selectbox("Jenis Fasilitas Kredit", ["KPR (Promo + Floating)", "KUR / Kredit Tetap"])
        st.markdown("---")
        st.subheader("Parameter Pinjaman")
        plafon = st.number_input("Plafon Pinjaman (Rp)", min_value=0, value=850000000, step=50000000, format="%d")
        tenor_bulan = st.slider("Tenor Pinjaman (Bulan)", min_value=12, max_value=360, value=240, step=12)
        
        if "KPR" in jenis_kredit:
            bunga_promo = st.number_input("Suku Bunga Promo Awal (% p.a)", value=db_rates[kategori_bank]["KPR_Promo"], step=0.01)
            masa_promo_thn = st.number_input("Lama Masa Promo (Tahun)", value=3, step=1)
            bunga_floating = st.number_input("Suku Bunga Floating (% p.a)", value=db_rates[kategori_bank]["KPR_Float"], step=0.01)
            masa_promo_bln = masa_promo_thn * 12
            tipe_kredit = "floating"
        else:
            bunga_tetap = st.number_input("Suku Bunga Efektif (% p.a)", value=6.00, step=0.01)
            tipe_kredit = "fixed"
            
    with col2:
        saldo = plafon
        data_jadwal = []
        for bulan in range(1, tenor_bulan + 1):
            sisa_tenor = tenor_bulan - bulan + 1
            if tipe_kredit == "floating":
                bunga_bln = (bunga_promo if bulan <= masa_promo_bln else bunga_floating) / 100 / 12
                cicilan = (saldo * (bunga_bln * (1 + bunga_bln)**sisa_tenor) / ((1 + bunga_bln)**sisa_tenor - 1)) if bunga_bln > 0 else saldo/sisa_tenor
            else:
                bunga_bln = bunga_tetap / 100 / 12
                cicilan = (plafon * (bunga_bln * (1 + bunga_bln)**tenor_bulan) / ((1 + bunga_bln)**tenor_bulan - 1)) if bunga_bln > 0 else plafon/tenor_bulan
            
            porsi_bunga = saldo * bunga_bln
            porsi_pokok = cicilan - porsi_bunga
            saldo -= porsi_pokok
            data_jadwal.append([bulan, cicilan, porsi_pokok, porsi_bunga, max(0, saldo)])
        
        df_jadwal = pd.DataFrame(data_jadwal, columns=["Bulan", "Total Angsuran", "Porsi Pokok", "Porsi Bunga", "Sisa Pinjaman"])
        
        # Tampilkan ringkasan biaya pinjaman keseluruhan
        total_pembayaran_kredit = df_jadwal["Total Angsuran"].sum()
        total_beban_bunga = df_jadwal["Porsi Bunga"].sum()
        
        st.subheader(f"Total Kewajiban Pinjaman Hingga Lunas: :red[Rp {total_pembayaran_kredit:,.0f}]")
        
        cm1, cm2 = st.columns(2)
        cm1.metric("Total Pokok Hutang", f"Rp {plafon:,.0f}")
        cm2.metric("Total Biaya Bunga (Cost of Credit)", f"Rp {total_beban_bunga:,.0f}")
        
        fig_pinjaman = go.Figure()
        fig_pinjaman.add_trace(go.Bar(x=df_jadwal["Bulan"], y=df_jadwal["Porsi Pokok"], name="Porsi Pokok", marker_color='#00CC96'))
        fig_pinjaman.add_trace(go.Bar(x=df_jadwal["Bulan"], y=df_jadwal["Porsi Bunga"], name="Porsi Bunga", marker_color='#EF553B'))
        fig_pinjaman.update_layout(barmode='stack', template="plotly_dark", height=350, margin=dict(l=0, r=0, t=30, b=0))
        st.plotly_chart(fig_pinjaman, use_container_width=True)

# ==========================================
# TAB 2: SIMULASI INVESTASI BERTAHAP
# ==========================================
with tab2:
    st.header(":violet[Simulasi Investasi Bertahap]")
    col3, col4 = st.columns([1, 3])
    with col3:
        st.session_state.modal_awal = st.number_input("Modal Awal (Rp)", value=st.session_state.modal_awal, step=10000000)
        st.session_state.tambahan_tahunan = st.number_input("Suntikan Dana per Tahun (Mulai Thn ke-2)", value=st.session_state.tambahan_tahunan, step=5000000)
        st.session_state.dividen_tahun = st.number_input("Target Pertumbuhan Tahunan (%)", value=st.session_state.dividen_tahun, step=0.1)
        lama_investasi = st.slider("Lama Investasi (Tahun)", 1, 50, 20)
        
    with col4:
        data_inv = []
        saldo_running = st.session_state.modal_awal
        total_modal_disetor = st.session_state.modal_awal
        
        for t in range(1, lama_investasi + 1):
            if t > 1: 
                saldo_running += st.session_state.tambahan_tahunan
                total_modal_disetor += st.session_state.tambahan_tahunan
            saldo_running *= (1 + st.session_state.dividen_tahun/100)
            profit_murni = saldo_running - total_modal_disetor
            data_inv.append([t, total_modal_disetor, profit_murni, saldo_running])
            
        df_invest = pd.DataFrame(data_inv, columns=["Tahun", "Total Modal Disetor", "Akumulasi Profit", "Total Portofolio"])
        st.subheader(f"Total Nilai Portofolio Akhir: :green[Rp {saldo_running:,.0f}]")
        st.line_chart(df_invest.set_index("Tahun")[["Total Modal Disetor", "Total Portofolio"]])

# ==========================================
# TAB 3: ANALISIS PERSILANGAN (CROSSOVER SINKRON)
# ==========================================
with tab3:
    st.header(":red[Analisis Persilangan: Hasil Dividen vs Beban Pinjaman Aktual]")
    st.write("Grafik ini mengukur secara murni perbandingan akumulasi profit investasi (Garis Hijau) melawan uang yang Anda bakar ke bank untuk cicilan (Garis Merah).")
    
    max_tahun = max(lama_investasi, math.ceil(tenor_bulan / 12))
    data_cross = []
    
    # Reset kalkulasi investasi internal untuk mencakup seluruh tenor pinjaman jika lebih panjang
    saldo_cross = st.session_state.modal_awal
    modal_cross = st.session_state.modal_awal
    
    for thn in range(1, max_tahun + 1):
        # 1. Hitung ulang profit murni tahunan secara sinkron
        if thn > 1 and thn <= lama_investasi:
            saldo_cross += st.session_state.tambahan_tahunan
            modal_cross += st.session_state.tambahan_tahunan
        elif thn > lama_investasi:
            pass # Selesai masa top-up, uang didiamkan menggulung murni
            
        saldo_cross *= (1 + st.session_state.dividen_tahun/100)
        profit_investasi_murni = saldo_cross - modal_cross
        
        # 2. Hitung akumulasi cicilan yang dibakar ke bank hingga akhir tahun tersebut
        bulan_berjalan = min(thn * 12, tenor_bulan)
        if bulan_berjalan > 0:
            uang_yang_dibakar = df_jadwal.head(bulan_berjalan)["Total Angsuran"].sum()
        else:
            uang_yang_dibakar = df_jadwal["Total Angsuran"].sum() # Jika sudah lunas, bebannya flat di titik maksimal
            
        data_cross.append([thn, profit_investasi_murni, uang_yang_dibakar])
        
    df_cross = pd.DataFrame(data_cross, columns=["Tahun", "Akumulasi Dividen", "Uang Yang Dibakar (Cicilan)"])
    
    fig_cross = go.Figure()
    fig_cross.add_trace(go.Scatter(x=df_cross["Tahun"], y=df_cross["Akumulasi Dividen"], name="Akumulasi Keuntungan (Profit)", line=dict(color='#00CC96', width=3.5)))
    fig_cross.add_trace(go.Scatter(x=df_cross["Tahun"], y=df_cross["Uang Yang Dibakar (Cicilan)"], name="Total Setoran ke Bank (Beban)", line=dict(color='#EF553B', width=3.5)))
    
    fig_cross.update_layout(
        title="Kurva Break-Even (Keuntungan Bersih vs Uang yang Dibakar)",
        xaxis_title="Tahun Ke-",
        yaxis_title="Nominal (Rp)",
        hovermode="x unified",
        template="plotly_dark",
        margin=dict(l=0, r=0, t=40, b=0)
    )
    st.plotly_chart(fig_cross, use_container_width=True)
    
    # Analisis Titik Temu Murni
    df_cross['Spread'] = df_cross['Akumulasi Dividen'] - df_cross['Uang Yang Dibakar (Cicilan)']
    titik_temu = df_cross[df_cross['Spread'] > 0]
    
    if not titik_temu.empty:
        tahun_temu = titik_temu.iloc[0]['Tahun']
        st.success(f"Strategi berhasil mencapai **Break-Even Murni** pada **Tahun ke-{tahun_temu:.0f}**. Pada tahun ini, profit murni hasil investasi Anda telah resmi mendominasi seluruh biaya pinjaman yang dikeluarkan.")
    else:
        st.warning("Keuntungan portofolio belum mampu menutupi total biaya cicilan bank hingga akhir periode analisis.")
